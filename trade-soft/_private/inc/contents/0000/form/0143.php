<?php 
    ...
    $report = ($lang ?  "বিক্রেতার তালিকা " : "Supplier List");
	
	$sup = $qur->custom_select_query("SELECT idcompany, company_name, company_phone, company_address,company_balance, company_creditlimit FROM company WHERE company_type=2 ORDER BY company_name;");
	$n = count($sup);
	?>
	...
	<? for($i=0; $i<$n;$i++){ ?> 
					<tr>
					  ...
					  <td class="action">
							<button type ='button'  class='inactive page-loader box-entry link0144 btn btn-info ' data =  "&id=<?php echo $sup[$i][0]; ?>&ln=<?php echo $lang; ?> "><?php echo ($lang == 1 ? "সংশোধন করুন " : "Edit");?></button>
					 </td>
