<?php
    ...
    for($i=0; $i<$n; $i++){
		if(post('sales_product_'.$i)){
			...
			$querya[$i][3] = array($pv1,  req_date('d1'),  post('sales_product_'.$i),  post('sales_quantity_'.$i),  post('sales_cost_'.$i),  post('v3'),  8,  $status==2 ? 1 : 0 );
    ...
    if($flag){                
		...
		$query = $query . ") VALUES (";
		$query = $query . $v1 .", ". post('sales_discount') .", ". post('sales_discount')*($total/100)." ,'". post('sales_comments') ."',  1, ". ($idstaff?  $idstaff : 'null') .",  ".post('sales_by').", '  " . post('sales_dlv')."    ' ,".$cat.",  ".post('sales_trns')." ,  ".post('v3')." ) ;";
