<?php
	...
	$cn = $qur->custom_select_query("SELECT contact_number FROM sales_details WHERE idsales = $sid LIMIT 1");

	$flag = $qur->execute("DELETE FROM stock_product_output WHERE idstock_product_output IN (SELECT idstock_product_output FROM sales_stock_product_output WHERE idsales = $sid );");
	//$ret_msg = $ret_msg."DELETE FROM stock_product_output WHERE idstock_product_output IN (SELECT idstock_product_output FROM sales_stock_product_output WHERE idsales = $sid );";
	if($flag){
		$flag = $qur->execute( "DELETE FROM sales WHERE sales_status=0 AND idsales= $sid;" );
		//$ret_msg = $ret_msg."DELETE FROM sales WHERE sales_status=0 AND idsales= $sid;";
	}
    ...
    $v2 = req('v2');
     ...
     if($flag){          
		...
		$query1[3] = array($v1,  $v2,  req_date('d1'),  0,  post('v4'), 0);
    ...
    $i = 0;
    for($i=0; $i<$n; $i++){
		if(post('sales_product_'.$i) &&  post('sales_quantity_'.$i)){
    ...
    if($flag){                
			...
			$queryax[1] = 'idsiiisididsss';
			$queryax[2] = array( "idsales", "sales_discount_f_rate",  "sales_comments", "sales_ispurchase_return", "iduser", "idstaff", "sales_delivery_address", "idproduct_catagory", "sales_transport", "idpestablishment", "sales_discount_p_rate", "sales_gps_a", "sales_gps_b", "contact_number" ); 
			$queryax[3] = array($v1, post('sales_discount_fitt') , post('sales_comments') , 0 , $idstaff ?$idstaff: 0  , post('sales_by') ,  post('sales_dlv')  ,$cat , post('sales_trns') , post('v3') , post('sales_discount_pipe') , post('loca'), post('locb'),$cn[0][0] );

		
			$flag = $qur->insert_query_by_array($queryax);
...
SELECT 2, sales_discount_f_rate FROM sales_details WHERE idsales = $v1
		...
		$qrxi1 = "UPDATE sales_details SET sales_discount_f_ammount = ".$dx[1][2]." , sales_discount_p_ammount = ".$dx[0][2]." WHERE idsales = $v1";
		$qrxi2 = "UPDATE sales SET sales_ammount = ".($dx[0][1] + $dx[1][1] + post('sales_trns') - $dx[0][2] - $dx[1][2]   )." WHERE idsales = $v1";
		if($flag){
...
/*---------------------------------------------------------------------------------------*/
        ...
        $link[$li++]['type'] = 'inactive page-loader box-entry link0306 btn btn-info';
