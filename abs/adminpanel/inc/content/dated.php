    ...
    if($_POST[$sc.'submit']){
		if($_POST['heading']&&$_POST['content']){
			...
			if($q)
			{
				...
				$q=mysql_query("INSERT INTO dated VALUES('','$text_data_id','$page_id',CURDATE())");
			}
			if($q){
					mysql_query("COMMIT");
			}
		...
		elseif($_POST['text_data_id_r']){
				...
				if($q)
				{
					$q=mysql_query("DELETE FROM dated WHERE page_id='".$page_id."' AND text_data_id='$text_data_id'");
				}
				if($q)
				{
