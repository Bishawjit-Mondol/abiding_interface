<?php
	...
	$flag = $qur->execute("DELETE FROM stock_product_input WHERE idstock_product_input IN (SELECT idstock_product_input FROM purchase_stock_product_input WHERE idpurchase = $pid );");
	$ret_msg = $ret_msg."DELETE FROM stock_product_input WHERE idstock_product_input IN (SELECT idstock_product_input FROM purchase_stock_product_input WHERE idpurchase = $pid );";
	if($flag){
		$flag = $qur->execute( "DELETE FROM purchase WHERE purchase_status=0 AND idpurchase= $pid;" );
		$ret_msg = $ret_msg."DELETE FROM purchase WHERE purchase_status=0 AND idpurchase= $pid;";
	}
    ...
    for($i=0; $i<$n; $i++){
		if(post('purchase_product_'.$i)){
			...
			if($fc_flag){
				...
				$querya[$i][3] = array($pv1,  req_date('d1'),  post('purchase_product_'.$i),  post('purchase_quantity_'.$i),  post('purchase_cost_'.$i)*req('d_rate'),  post('v3'),  1,  $status );
				$total = $total + ( post('purchase_quantity_'.$i)*post('purchase_cost_'.$i)*req('d_rate'));
			}
			else{
				$querya[$i][0] = 'stock_product_input';
				$querya[$i][1] = 'isiddiii';
				$querya[$i][2] = array( 'idstock_product_input' , 'stock_product_input_date' , 'idproduct' , 'stock_product_input_qunatity' , 'stock_product_input_price' , 'idpestablishment' , 'stock_product_input_type' , 'stock_product_input_status' ); 
				$querya[$i][3] = array($pv1,  req_date('d1'),  post('purchase_product_'.$i),  post('purchase_quantity_'.$i),  post('purchase_cost_'.$i),  post('v3'),  1,  $status );
    ...
    if($flag){                
			...
			if($fc_flag){
				...
				$queryax[3] = array($v1, post('purchase_discount_fitt') , post('purchase_comments') , 0 , $idstaff ?$idstaff: 0  , post('purchase_by') , $cat , post('purchase_trns') , post('v3') , post('purchase_discount_pipe'), req_date('d1'), req_date('d1'), req('d_rate') , 1  );

			}
			else{
				$queryax[0] = 'purchase_details';
				$queryax[1] = 'idsiiiididss';
				$queryax[2] = array( "idpurchase", "purchase_discount_f_rate",  "purchase_comments", "purchase_issales_return", "iduser", "idstaff",  "idproduct_catagory", "purchase_transport", "idpestablishment", "purchase_discount_p_rate", "purchase_details_billing_date", "purchase_details_chalan_date"); 
				$queryax[3] = array($v1, post('purchase_discount_fitt') , post('purchase_comments') , 0 , $idstaff ?$idstaff: 0  , post('purchase_by') , $cat , post('purchase_trns') , post('v3') , post('purchase_discount_pipe'), req_date('d1'), req_date('d1') );
...
SELECT 2, purchase_discount_f_rate FROM purchase_details WHERE idpurchase = $v1
		...
		$qrxi2 = "UPDATE purchase SET purchase_ammount = ".($dx[0][1] + $dx[1][1] + post('purchase_trns') - $dx[0][2] - $dx[1][2] ).", purchase_status = ".$status." WHERE idpurchase = $v1";
		if($flag){
