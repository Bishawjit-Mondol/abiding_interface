<?php
    ...
    if($flag){  
		...
		if($v1 == 1){
			...
			$query1[3] = array( $v0, $date, $v4, $v5*$v1, $v3, $v6, $status, $v11, 1);  //transaction_type means 'method'
			...
			$flag = $qur->insert_query_by_array($query1);
			$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=(2222) </br>" ).$qur->queryPrint($query1);
		}
    }
	if($flag){  
	...
	if($flag && $v6==2){  
		...
		$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=3 </br>" ).$qur->queryPrint($query3);
			...
			$query3[1] = 'isssss';
			$query3[2] = array('idtransaction','transaction_cheque','transaction_cheque_date','transaction_bank','transaction_branch','transaction_comments'); 
			$query3[3] = array( $v0, $det[0][0], $date2, $v7, $v9, $v10 );
                   
			$flag = $qur->insert_query_by_array($query3);
			$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=4 </br>" ).$qur->queryPrint($query3);
	
			if($flag){
				$query4[0] = 'transaction_cheque_leaf';
				$query4[1] = 'ii';  
				$query4[2] = array( 'idtransaction', 'idcheque_leaf' ); 
				$query4[3] = array( $v0, $v8 );
                   
				$flag = $qur->insert_query_by_array($query4);
				$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=5 </br>" ).$qur->queryPrint($query4);
			}
		
			if($flag){
				$queryx = "UPDATE cheque_leaf  SET cheque_leaf_status = 1 WHERE idcheque_leaf = ".$v8 ;
				$flag = $qur->execute($queryx);
				$ret_msg = $ret_msg.''.$queryx;
			}
		}
    ...
    if($flag){
		...
		*/
        ...
        $qur->con->rollback(); 
        $ret_msg = ($lang ?  " দু:খিত, ব্যর্থ হয়েছে ।" : " Failed to do." ).$ret_msg.''.get_insert_query($query6);
    }
    unset($query);    
