...
//	$details = $qur->custom_select_query("CALL ___show_sales_order($c1,$c2,'".$d2."','".$d1."')");
	...
	if($type == 1){
		...
		//echo "CALL ___show_ppr_sales_order($c1,$c2,'".$d2."','".$d1."')";
	}
	else if($type == 2){
		$details = $qur->custom_select_query("CALL ___show_upvc_sales_order($c1,$c2,'".$d2."','".$d1."')");	
		//echo "CALL ___show_upvc_sales_order($c1,$c2,'".$d2."','".$d1."')";
	}
...
?>
	...
	<? for($i=0; $i<$n;$i++){ ?> 
					<tr>
					  ...
					  <td  class = 'action'>
							<button type ='button'  class='inactive page-loader box-entry link0146 btn btn-info confirm_sale_<?php echo $details[$i][0];?>' data = "&id=<?php echo $details[$i][0];?>&ch_date=<?php echo $ch_date;?>&ln=<?php echo $lang; ?>&rtype=1"><? echo ($lang? ' নিশ্চিত করুন' :' Chalan');?></button>
