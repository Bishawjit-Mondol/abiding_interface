<?php 
	...
	$label_6 =  ($lang == 1 ? "তারিখ " : "Cash / Bank "); 
    $label_3 =  ($lang == 1 ? "তারিখ " : "Details"); 
    $label_4 =  ($lang == 1 ? "মূল্য" : "Received Amount ");
	$label_5 =  ($lang == 1 ? "পদক্ষেপ " : "Action");
	$report = ($lang ?  "নতুন পন্য বিক্রয়ের অর্ডার" : "Show");
	...
	$opening_bl = 0.00;
	$total = 0.00;
	$details = $qur->custom_select_query("CALL ___show_recived_statement( $c1, $c2,'".$d2."','".$d1."')");
	$n = count($details);
?>
			<?php include('rh.php'); ?>
	...
	<?php for($i=0; $i<$n;$i++){ ?> 
					<tr>
					  ...
					  <td  class = 'action'>
							<button type ='button' class='btn btn-info confirm_sal' sal_id = '<?php echo $details[$i][0];?>' >Confirm</button>
