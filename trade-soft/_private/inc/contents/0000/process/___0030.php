  ...
  if(req('submit_type')=='2'){
    ...
    $p1 = 0;
     ...
     if($flag){          
		...
		$query1[3] = array($v1,  post('v2'),  req_date('d1'),  0,  $status );
    ...
    for($i=0; $i<$n; $i++,$pv1++){
		if(post('purchase_product_'.$i)){
			...
			$query2[3] = array($pv1,  req_date('d1'),  post('purchase_product_'.$i),  post('purchase_quantity_'.$i),  post('purchase_cost_'.$i),  post('v3'),  2,  $status );
    ...
    if($flag){                
		...
		$query = $query . ") VALUES (";
		$query = $query . $v1 .", ". post('purchase_discount') .", ". post('purchase_discount')*($total/100)." ,'". post('purchase_comments') ."',  0, ". ($idstaff?  $idstaff : 'null') .",  ".post('purchase_by').",  ".$cat.",  ".post('purchase_trns')." ,  ".post('v3')." ) ;";
    ...
    if($flag){
        $qur->con->commit();
        $ret_msg = ($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Successfully Done." ).$msg;
