<?php 
    ...
    $query1[3] = array( $v0, $date, 1, $v5*$v1, $v3, $v6, $status, 1);  // transaction_type means 'method'
    ...
    $query1[1] = 'isidiiii';
    $query1[2] = array('idtransaction','transaction_date','idmestablishment','transaction_ammount','idtransaction_head','transaction_type','transaction_status','iduser'); 
    $query1[3] = array( $v0, $date, $v4, $v5*$v1, $v3, $v6, $status, 1);  // transaction_type means 'method'
             								     //      
        $flag = $qur->insert_query_by_array($query1);
		$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=1 </br>" ).$qur->queryPrint($query1);;
		unset($query1);
	...
	elseif($flag && $v6==1){ 
		...
		$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=7 </br>" ).$qur->queryPrint($query6);
	}
	if($idpurchase){
		if($flag){  
			$queryTransactionIdPurchase[0] = 'transaction_purchase';
			$queryTransactionIdPurchase[1] = 'ii';
			$queryTransactionIdPurchase[2] = array('idtransaction','idpurchase'); 
			$queryTransactionIdPurchase[3] = array( $v0, $idpurchase);
                   
			$flag = $qur->insert_query_by_array($queryTransactionIdPurchase);
			$ret_msg = $ret_msg.($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success=8 </br>" ).$qur->queryPrint($queryTransactionIdPurchase);
		//unset($queryTransactionIdPurchase);
		}
	}
