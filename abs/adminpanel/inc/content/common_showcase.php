				...
				if($q&&$text_data_id&&$_FILES["newimage".$i]["name"])
				{
					...
					if(!$fileerror){
						...
						if(($imgtype_id!=null)&&($filesize<500000)){
							...
							$q=mysql_query("INSERT INTO img VALUES('','$caption','$page_id','$text_data_id','$imgtype_id','$link','$border','$priority')");
...
<a class="button" onClick="showit(4)">Slides</a>
	...
	<table class="structured">
		...
		<tr>
		  <td>
				...
				<form action="index.php?page=<?php echo $page;?>&&p=edit" method="POST">
...
<div id="sud4">
	...
	<?php
		...
		{
			echo "<td>";
				display_image_preview($data[0]);
				echo "<form method='POST' action='index.php?page=common_edit_image'>";
					echo "<input type='hidden' name='img_id' value='".$data[0]."'/>";
					echo "<input type='hidden' name='page_id' value='".$page_id."'/>";
					echo "<input type='submit' name='edit' value='Edit' onClick='showloading();'/>";
				echo "</form>";
			echo "</td>";
			if(($count%2)==0)
				echo "</tr><tr>";
