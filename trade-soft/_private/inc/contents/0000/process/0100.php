<?
    ...
    $qur->execute('START TRANSACTION');   // update a status 1 korte hobe?(propose or confirm)
	
    if($flag){  
		$ret_msg = "<br/>"."UPDATE `transaction` SET transaction_ammount= $v5*$v13 , transaction_date = '".$d1."', transaction_status = 1 WHERE idtransaction=$v12";
		$flag = $qur->execute("UPDATE `transaction` SET transaction_ammount= $v5*$v13 , transaction_date = '".$d1."', transaction_status = 1 WHERE idtransaction=$v12");
