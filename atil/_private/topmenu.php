<ul>
	...
	while ($datat = mysql_fetch_assoc($qt)) {
		...
		if ($datat['pagetype_id'] == 3) {
			...
			if (mysql_num_rows($qcatm) > 0) {
				...
				while ($datatcatm = mysql_fetch_assoc($qcatm)) {
					...
					if (mysql_num_rows($qctd) > 0) {
						...
						$qtdd = mysql_query("SELECT id, heading FROM text_data WHERE " . $qcond . " AND hidden='0' ORDER BY priority DESC");
			...
			echo "<ul>";
			$qt2 = mysql_query("SELECT id, heading FROM text_data WHERE page_id='" . $datat['id'] . "'  AND hidden='0' ORDER BY priority DESC LIMIT 10");
