<?
	...
	$label_8 =  ($lang == 1 ? "পূর্বের দেনা " : "Previous Debit"); 
	...
	$details = $qur->custom_select_query("SELECT idcompany,___find_company_name(idcompany) COMPANY, SUM(p_amount), SUM(pr_amount), SUM(p_amount)-SUM(pr_amount) as net_pur, SUM(tran),___find_previous_purchase_balance(idcompany, '$d2')
FROM (
  SELECT idcompany, SUM(purchase_ammount) p_amount, 0 pr_amount, 0 tran FROM purchase
  LEFT JOIN purchase_details USING(idpurchase) WHERE purchase_status=1 AND purchase_issales_return=0 AND purchase_date BETWEEN '$d2' AND '$d1'  GROUP BY idcompany
  UNION
  SELECT idcompany, 0 p_amount, SUM(sales_ammount) pr_amount, 0 tran FROM sales
  LEFT JOIN sales_details USING(idsales) WHERE sales_status=2 AND sales_ispurchase_return=1 AND sales_date BETWEEN '$d2' AND '$d1' GROUP BY idcompany
  UNION
  SELECT idcompany, 0 p_amount, 0 pr_amount, SUM(transaction_ammount) tran
  FROM transaction_company LEFT JOIN `transaction` USING(idtransaction) WHERE (idtransaction_head=13 OR idtransaction_head=16) AND transaction_status=1 AND transaction_date BETWEEN '$d2' AND '$d1' GROUP BY idcompany
...
?>
	...
	<?php	$pur=0; $pur_rt=0; $net_pur=0; $dt=0; $pb=0; $total=0; 
			for($i=0; $i<$n;$i++){ ?> 
					<tr>
					  <td><?=$inp->cb($i+1);?></td>
