<?php 
	...
	if(req('rtype')=='1'){	
		...
		if($flag){
			$flag = $qur->execute("UPDATE sales_details SET sales_details_billing_date= '".$b_date."' WHERE idsales= $id;");
		}
		
		if($flag){   ?>
			<script type = 'text/javascript'>
			$(".confirm_bill_<?php echo $id; ?>").hide('slide');
			</script>
    	     <?php   }
	}
	...
	if(!validateDate( req_date('bl_date') , 'Y-m-d')){
		$flag = false;
	    echo "<h3>".($lang ?  "বিলে তারিখ সঠিক নয়" : "Bill Date is not Correct.")."</h3>";
	}
	...
	<table class='roundangled20 <? if(!$fpp){ echo 'pagebreak'; } ?> '  >
		...
		<tr class='topinfo'>
			  ...
			  <td width = '60%' >
				  <table>
  					   <tr>
					      <td> <?php echo($lang == 1 ? "ক্রেতা ও ঠিকানা   " : "Customer Name & Address  ");?> :   <?php echo $det[0][13]."<br/>".$det[0][14];?>
					      </td>
					  </tr>
					<tr>
						<td> <?php echo($lang == 1 ? "পৌছানোর ঠিকানা" : "Delivery Address");?> :    <?php echo $doid;?> </td>
...
<thead>
			<tr>
			<td><?php echo ($lang == 1 ? "ক্রম " : "Sl");?></td>
			<td><?php echo ($lang == 1 ? "পন্যের কোড " : "Item<br/>Code");?></td>
			<td><?php echo ($lang == 1 ? "পন্য" : "Product");?></td>
			<td><?php echo ($lang == 1 ? "ইউনিট" : "Unite");?></td>
			<td><?php echo ($lang == 1 ? "পরিমান " : "Qty.");?></td>
			<td><?php echo ($lang == 1 ? "দর" : "Rate");?></td>
...
<?
		...
		{ $error = $qur->custom_select_query("SELECT @software_error"); 
     			echo ($lang ?   " ব্যর্থ হয়েছে " : "Failed To Do :  ").$error[0][0]."UPDATE sales SET sales_status= 2 WHERE idsales= $id;;UPDATE sales_details SET sales_details_billing_date= '".$b_date."' WHERE idsales= $id;";
		}
