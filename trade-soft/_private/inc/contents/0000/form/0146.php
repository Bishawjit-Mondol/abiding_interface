<?php 
	...
	if(!validateDate( req_date('ch_date') , 'Y-m-d')){
		$flag = false;
	    echo "<h3>".($lang ?  "চালানের তারিখ সঠিক নয়" : "Chalan Date is not Correct.")."</h3>";
	}
...
<?php 
	...
	if(true){
		...
		company_name,  company_address, contact_number FROM (
SELECT * FROM (
...
LEFT JOIN pestablishment_product USING(idproduct) group by idproduct;");
		...
		$poid = $qur->custom_select_query("SELECT poid FROM sales_poid s WHERE idsales = $id");
		$poid = $poid[0][0];
		if(!$poid){
			$poid = 'Nill';
		}
	...
	<tbody>
		<tr>
			<td>
				<table height="180px">
					<tr>
						<td width="352px"> <?php echo($lang == 1 ? " চালানের তারিখ" : " Chalan Date");?> :  <?php echo ( $det[0][10] && $det[0][10] != "0000-00-00"  ?  dateconvert($det[0][10]) : " Not Yet" );?></td>					</tr>					
					
					<tr>
						<td> <?php echo($lang == 1 ? "পি.ও আইডি" : "P.O.ID");?> :    <?php echo $poid;?></td>
					</tr>
			...
			<td>
				<table height="180px">
					<tr>
						<td width="354px" valign="top"> <?php echo($lang == 1 ? "ক্রয়কারী ও ঠিকানা   " : "Customer Name & Address  ");?> :   <?php echo $det[0][13]."<br/>".$det[0][14];?></td>
					</tr>
					<tr>
...
<table class="table table-striped table-hover report-table1" border = '1' width = '100%'>
	<thead>
		<tr>
			...
			<td><?php echo ($lang == 1 ? "বর্তমান মজুদ" : "Current Stock");?></td>
...
<?php  $i = 0; $tot = 0; foreach($det2 as $d){ ?>
		<tr>
			...
			<td><?php echo $inp->cb($d[4]);?></td>
			<td><?php 
				
				if( $d[3]>$d[4] ){ echo ($d[3]-$d[4]).'  amount is shorted';}
...
<? if($det[0][10] && $det[0][10] != "0000-00-00"){ ?>
<div class="form-group">
	<div class = 'col-lg-12'>
								<div class="input-group margin-bottom-sm pull-right">
									<button class="btn btn-info print-chalan-secial"> <i class = 'fa fa-print'></i> Print Chalan  </button>
								</div>                  
							</div>        
</div>
<? } ?>
...
<div class = 'chalan_print_try <? if(!$fpp){ echo 'pagebreak'; } ?>' >
	<table class='roundangled20  <? if(!$fpp){ echo 'pagebreak'; } ?>'>
		...
		<tr><td align = 'center' colspan = '2' class='banner'>Delivery Challan : AG<?=$det[0][0];?></td></tr>
		<tr class='topinfo'>
			  ...
			  <td width = '40%'>
				  ...
				  <table>
					  <tr>
						<td width="352px"> <?php echo($lang == 1 ? " চালানের তারিখ" : " Chalan Date");?> :  <?php echo ( $det[0][10] && $det[0][10] != "0000-00-00"  ?  dateconvert($det[0][10]) : " Not Yet" );?></td>		
...
?>
				  <table>
					...
					<tr>
					<td width="354px" valign="top"> <?php echo($lang == 1 ? "ক্রেতা ও ঠিকানা   " : "Customer Name & Address  ");?> :   <?php echo $det[0][13]."<br/>".$det[0][14];?></td>
					</tr>
