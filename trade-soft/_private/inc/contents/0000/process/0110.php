<?php
	...
	$flag = $qur->execute("DELETE FROM stock_product_input WHERE idstock_product_input IN (SELECT idstock_product_input FROM purchase_stock_product_input WHERE idpurchase = ".req('pur_id')." )");
	//$msg = $msg.'</br>'.("DELETE FROM stock_product_input WHERE idstock_product_input IN (SELECT idstock_product_input FROM purchase_stock_product_input WHERE idpurchase = ".req('pur_id')." )");
	
	if($flag){
		$flag = $qur->execute("DELETE FROM purchase WHERE idpurchase = ".req('pur_id'));
		//$msg = $msg.'</br>'.("DELETE FROM purchase WHERE idpurchase = ".req('pur_id'));
	}
	$v1 = req('pur_id');   
    ...
    $p1 = 0;
     ...
     if($flag){          
        ...
        //$msg = $msg.'</br>'.$qur->queryPrint($query1);
    ...
    for($i=0; $i<$n; $i++,$pv1++){
		if(post('purchase_product_'.$i)){
			...
			if($flag){              
				    ...
				    //$msg = $msg.'</br>'.$qur->queryPrint($query2);
			...
			if($flag){                
				        ...
				        //$msg = $msg.'</br>'.$qur->queryPrint($queryb3);
    ...
    if($flag){
        ...
        //$msg = $msg.'</br>'.$qur->queryPrint($queryx);
    ...
    if($flag){                	
        ...
        //$msg = $msg.'</br>'.$qur->queryPrint($q);
		//$ret_msg = ($lang ?  "সফলভাবে সম্পন্ন হয়েছে।" : "Success 6  " ).$ret_msg;
        unset($q);
...
/*---------------------------------------------------------------------------------------*/
        ...
        $link[$li]['address'] = $addr_exten1 = "&&id=".$id_purchase."&&ln=".req('ln');
