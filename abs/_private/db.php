<?php
	function dbconnect()
	{
		mysql_connect("localhost","abidingg_absu","0912AK_3") || die(mysql_error());
		//mysql_connect("localhost","root","") || die(mysql_error());
		mysql_select_db("abidingg_abs2") || die(mysql_error());
	...
	{
		echo "<div class='wrapper row5'><div class='rslides_container clear'><ul class='rslides clear' id='rslides'>";
	...
	function display_multi_image($page_id,$text_data_id)
	{
		...
		if($num>0)
		{
			echo "<div class='imgrholder'>";
			while($data=mysql_fetch_assoc($q))
			{
				if($data['link'])
				{
					echo "<br/><a href='".$data['link']."' target='_blank'>";
				}
				else 
				{
					echo "<br/><a href='index.php?page=largeview&&img_id=".$data['id']."&&data_id=".$text_data_id."&&backpage=".$_GET['page']."'>";
				}
				echo "<img src='images/db/".$data['id'].imgtype_by_id($data['imgtype_id'])."' alt='".$data['caption']."'";
				if($data['border'])
				{
				...
				if($data['caption'])
				{
					echo "<br/><b>".nl2br($data['caption'])."</b>";
				}
				echo "</a>";
				echo "<br/>";
			}
		...
		if($num>0)
		{
			if($num>1)
			{
				...
				echo "<div class='buttoncontainer'>";
					...
					while($url[$count])
					{
						echo "<td><a onmouseover='showit(".$count.")' href='index.php?page=largeview&&img_id=".$img_id[$count]."&&data_id=".$text_data_id."&&backpage=".$_GET['page']."'><img src='".$url[$count]."'/></a></td>";
						$count++;
					}
					echo "</tr></table>";
					if($num>5) 
	...
	function display_single_image()
	{
		...
		if(mysql_num_rows($q)>0)
		{
			...
			{	
				...
				echo $heading;
				echo "</h2></a>";
				echo "<a href='index.php?page=".$backpage."&&data_id=".$data_id."'><img src='images/db/".$img_id.imgtype_by_id($data['imgtype_id'])."' alt='".$data['caption']."'/></a>";
				echo "<br/><br/><h2>".nl2br($data['caption'])."</h2>";
				echo "<a href='index.php?page=".$backpage."&&data_id=".$data_id."' class='button large black rnd8'>Source page &raquo; ".$heading."</a>";
				echo "<center>";
			}
		}
	}
	...
	function get_pagination($perpage,$start,$pageurl,$qc)
	{
		...
		if($num_articles>$perpage)
		{
			...
			{
				$pagination=$pagination."<li><a href='".$pageurl."&&start=".$pre_start."' class='prev'>&laquo; Previous</a></li>";
				$count=1;
				for($i=0;$i<$num_articles;$i=$i+$perpage)
				{
					if($i==$start)
						$pagination=$pagination."<li class='current'><a href='".$pageurl."&&start=".$i."'>".$count."</a></li>";
					else
						$pagination=$pagination."<li><a href='".$pageurl."&&start=".$i."'>".$count."</a></li>";
					$count++;
				}
				$pagination=$pagination."<li><a href='".$pageurl."&&start=".$next_start."' class='next'>Next &raquo;</a></li>";
			}
