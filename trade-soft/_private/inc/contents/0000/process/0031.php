<?php
  ...
  if(req('submit_type')=='2'){
    ...
    if($flag){          
		...
		$queryvx1[0] = 'sales_poid';
		$queryvx1[1] = 'is';
		$queryvx1[2] = array( 'idsales', 'poid');
			
		$queryvx1[3] = array($v1,  $poid  );

        $flag = $qur->insert_query_by_array($queryvx1); 
    ...
    if($flag){                
			...
			$queryax[3] = array($v1, post('sales_discount_fitt') , post('sales_comments') , 0 , $idstaff ?$idstaff: 0  , post('sales_by') ,  post('sales_dlv')  ,$cat , post('sales_trns') , post('v3') , post('sales_discount_pipe') , post('loca'), post('locb') );
...
SELECT 2, sales_discount_f_rate FROM sales_details WHERE idsales = $v1
		...
		$qrxi1 = "UPDATE sales_details SET sales_discount_f_ammount = ".$dx[1][2]." , sales_discount_p_ammount = ".$dx[0][2]." , contact_number = '".post('cont_num')."' WHERE idsales = $v1";
